...
| DESK-006 | Video/music autoplay and shuffle | DM autoplay/shuffle actions; video/audio controls. The first music subset reads typed `AutoPlayMusic` and `ShuffleMusic`, starts music only when selected-game video policy does not take precedence, exposes an interactive shared Qt player in game details, refreshes it with the media revision, and stops it before launching games or additional applications. Background/platform music, global shortcuts, notification integration, and native Windows/macOS multimedia execution remain open |
