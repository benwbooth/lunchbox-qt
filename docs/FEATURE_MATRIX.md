...
| BB-017 | Favorites, star ratings, playlists, random/discovery, and related games | BV popups/menu actions/discovery model. The first native game-action subset recovers all six favorite/rating visibility and ordering settings with observed defaults, preserves independent integer and floating rating fields, uses an explicit half-star clean-room policy, and stable-partitions favorites first. Wheel markers/details and game-menu Favorite/Rate actions honor their independent settings and existing security permissions. A centered Qt popup renders the recovered faded/bright five-star composition and accepts keyboard, mapped-controller, and pointer input. Favorite plus both rating fields are committed through one lossless platform transaction with an exact backup, committed reread, live model/revision refresh, completion retention, and unknown-XML retention. Pure policy/storage/security tests and a compiled locked-to-unlocked interaction validate denial, the rendered 4.5 popup, a 2.5 write, unfavoriting, two exact transaction states, immutable peers, and no recovery manifest. The shared Rust/CXX-Qt/QML path contains no runtime shell, Windows path logic, or OS-specific UI branch. Exact protected stepping/pointer rounding, custom-theme bindings, playlists, discovery/random behavior, related games, and native Windows/macOS Qt interaction remain open |
