...
frontends. The first `DESK-009` subset adds the recovered standard Find focus
command, exact `Ctrl+Alt+Q` Select Random Game command, and a separately guarded
Play Random action. The first `DESK-001` subset also switches LaunchBox between its
...
| LIB-013 | Search, sort, arrange, filters, random selection, and suggestions | DM arrange/search/random; DV search/suggestion/filter options; the port implements descriptive search plus 17 typed LaunchBox-keyed Arrange By modes with ascending/descending order, missing-last semantics, stable title/ID ties, transactional `Settings.xml` persistence, selection retention across resets, and random visible-game selection that avoids the current game when possible. LaunchBox exposes the recovered standard Find focus command, exact `Ctrl+Alt+Q` selection command, separate visible Select Random and guarded Play Random actions, and shell-free launch handoff by stable ID. BigBox exposes the shared selection state through its recovered Random Game input action and visible RANDOM control. The native BigBox Discovery Center adds six recovered ordered local slots and the exact Recently Added 360-day/minimum-five/maximum-25 contract. Its native 13.27 provider adapter fetches once in the background, validates/caches the recovered bounded response, appends priority-ranked before shuffled random lists, resolves manual database IDs/title/platform, and evaluates supported automatic Boolean/text/numeric/date criteria with LaunchBox field grouping. Offline service state retains local lists, and unsupported semantics reject the provider row. Protected local rankings, current live-service validation, and MAME remain open. Suggestions, dynamic/custom-field arrangement, grouped headings, and Random-as-an-arrangement remain open; the original protected random algorithm is not claimed |
...
| DESK-009 | Search focus, random play, and random selection | desktop commands/menu actions. The first native subset preserves the 13.27 distinction between Select Random Game and the separate `PlayRandomGameCommand`: Qt's standard Find shortcut focuses the real search field, `Ctrl+Alt+Q` selects without launching, and visible Select Random/Play Random controls share the typed visible-row selector. Play Random is guarded against load/write/launch/session overlap and hands the chosen stable ID to the common process boundary. The injected-entropy/current-game-exclusion policy is explicitly clean-room because protected bodies remain stubs. A compiled workflow activates both shortcut handlers, renders both controls, runs a mapped Windows-path portable process with exact argv, and verifies the two-write statistics/backup chain. Native Windows/macOS shortcut delivery remains a real-host gate |
...
| DESK-012 | Premium registration and BigBox mode entry | DM license/premium/BigBox actions; entitlement state in PA. The first native mode-entry subset adds a visible LaunchBox `Big Box Mode` action over one typed platform planner and a CXX-Qt controller gate. It refuses every active load/import/emulator scan or update/write/game launch/session/startup/shutdown/pause lifecycle, starts the sibling `bigbox` executable directly without a command shell, and carries only the live library, selected stable game, explicit host mappings, and UI/model state locations. An absolute peer override exists for packaging and conformance fixtures but is never forwarded. Pure tests freeze suffix-aware sibling selection, singleton validation, activity gates, and a strict allowlist that drops smoke, window, launch, and unknown arguments. A compiled Qt workflow activates the real control, launches a portable recorder, and compares all 12 forwarded arguments exactly. This does not invent or bypass LaunchBox premium entitlement: registration, license validation/storage, entitlement-driven availability, and native Windows/macOS Qt execution remain open |
...
| BB-018 | Desktop-mode handoff and application exit | BV desktop/exit menu actions. The first native handoff subset exposes a visible `DESKTOP MODE` control, preserves the selected stable game when returning to LaunchBox, and shares the direct-process planner and full activity gate with desktop mode entry. Both QML and Rust enforce the recovered `BigBoxExit` locked-mode permission before spawn; failure leaves BigBox running with an actionable status. Normal window/input exit already uses the same central permission gate and stops screensaver, Attract Mode, startup presentation, music, and marquee lifecycles on close. Compiled offscreen runs activate both frontend controls, require exact sanitized argv and live library/selection continuity, and prove configured locked BigBox creates no peer process and does not mutate settings. Exact protected handoff sequencing, premium-gated entry parity, packaged macOS bundle layout, and native Windows/macOS process/UI execution remain open |
