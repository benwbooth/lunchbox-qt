...
| BB-004 | Game details, images, video, 3D model, related games, and additional documents/apps | BV details/media/popups/actions; the first five port subsets open a keyboard-first full-screen details layer and a separate image-only layer from the selected stable-ID game, add front/back box flipping, add the shared interactive typed model viewer, and add manual/music actions. Details presents title/platform/status/ratings/statistics/metadata/notes beside actual image/video thumbnails, wraparound previous/next, autoplay, play/pause, mute, mapped Select-to-play/pause, distinct Play Game, and Back over the shared bounded media index. The image layer adds direct and nested entry, video filtering, image-type switching, bounded zoom/fit/pan, wheel/drag/directional/page/Select input, and context-preserving Back. The wheel honors `ShowGameMenuFlipBox`; the game menu honors `ShowGameMenuViewModelFullscreen` and opens a six-face Qt Quick 3D model through mapped Show Model, visible controls, mouse/wheel/keyboard input, and strict persisted rotation lock shared with LaunchBox. Resolved 13.27 model settings select functional box, DVD, jewel, or long-jewel proportions and colors and choose separate face media or ratio-based `Box - Full` construction. Settings-gated View Manual and Play Music actions exist in both the wheel menu and details; typed list/details autoplay, video-audio priority, repeat, shuffle, and volume drive the shared Qt audio player without restarting a user-paused same-game track. The shared desktop platform editor atomically updates the platform override seen by BigBox. Compiled 1280x720 scenarios drive the real controls through SVG, H.264, MP3/M3U, PDF local URLs, and Qt Quick 3D states; require game-over-platform model precedence, the expected jewel dimensions, and the full-scan ratio; visually check details, image viewer, back cover, model, and music-control renders; validate native regular files in Rust; and hash the complete media tree. Logo/front-spine/rotation material rendering, exact protected mesh/material/transition/camera behavior, related games and non-manual documents, custom theme layouts/transitions, global audio controls/notifications, and native Windows/macOS interaction remain open |
...
| BB-012 | Gamepad, keyboard, keyboard-automation, and mouse options | BV option pages and binding controls. The first native input vertical recovers all 59 persisted BigBox actions, every keyboard-setting name and four-slot convention, all observed nonzero WPF `Key` defaults, `EnableGamepad`, `UseAllControllers`, the `ControllerBinding` vocabulary, hold chords, and the exact 18 default BigBox controller rules. A typed Rust policy converts WPF integers to Qt portable sequences, groups duplicate sequences into ordered action candidates, parses configured rules without inventing malformed values, and feeds one central QML dispatcher for wheel/details/images/model/navigation/music/attract/screensaver/pause/volume actions. Native `gilrs` backends consume hot-plugged Linux, Windows, and macOS gamepads without a subprocess or shell; stick hysteresis, press edges, disconnect cleanup, and first-controller/all-controller policy live below QML. Ten pure tests freeze the 59-action order, four slots, defaults, conversion, duplicate grouping, malformed fallback, unsupported-rule accounting, holds/edges, disabled input, and controller ownership. A compiled offscreen scenario loads the real XML fixture and drives semantic Button1/Button2/D-pad/Button4 events through CXX-Qt and the actual dispatcher to open/close details, navigate stable IDs, and open/close images while byte-comparing both settings files and the media tree. Windows plus Intel/Apple Silicon Darwin cross-target gates compile the native backend. Keyboard automation/mouse-option pages, binding editors, raw `Button14` through `Button32` device mapping, exact protected active-controller selection, global input while another process owns focus, and real-device/native-Qt execution on Windows/macOS remain open |
