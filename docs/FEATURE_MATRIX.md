...
| DESK-004 | Image zoom, type switching, box flip, full-screen images/models | DM zoom/image/model/flip actions; the first two port subsets implement image-only full-screen views in LaunchBox and BigBox over native Qt file URLs, wraparound image-family switching, an explicit 100–400%/25% zoom policy, fit reset, bounded pan, wheel/drag/keyboard/button input, and focus-preserving return-to-details behavior. LaunchBox supports docked and native pop-out ownership plus direct `I` entry. Both LaunchBox and BigBox now select the box back through the recovered settings priority and expose real front/back controls, `F`, and a shared 220 ms Qt Y-axis transition; BigBox returns to the front when wheel selection changes, while LaunchBox retains per-tile state until Front is used. Compiled interactions activate and return through the real controls, render the back art, validate distinct native regular files in Rust, and prove whole-tree immutability. Exact original transition timing, 3D models, native game-controller bindings, and native Windows/macOS interaction stay open |
...
| BB-004 | Game details, images, video, 3D model, related games, and additional documents/apps | BV details/media/popups/actions; the first three port subsets open a keyboard-first full-screen details layer and a separate image-only layer from the selected stable-ID game, then add front/back box flipping to the game wheel. Details presents title/platform/status/ratings/statistics/metadata/notes beside actual image/video thumbnails, wraparound previous/next, autoplay, play/pause, mute, Enter-to-play, and Back over the shared bounded media index. The image layer adds direct and nested entry, video filtering, image-type switching, bounded zoom/fit/pan, wheel/drag/directional/page/Enter input, and context-preserving Back. The wheel honors `ShowGameMenuFlipBox`, exposes a real Flip/Show Front button plus the recovered `F` binding, resets on selection change, and shares the settings-prioritized native back image and 220 ms Qt transition with LaunchBox. Compiled 1280x720 scenarios drive the real controls through SVG and H.264 states, visually check details, viewer, and back-cover renders, validate native regular files in Rust, and hash the complete media tree. Native game-controller binding, exact protected transition timing, 3D models, related games, documents, additional-app actions, custom theme layouts/transitions, and native Windows/macOS interaction remain open |
