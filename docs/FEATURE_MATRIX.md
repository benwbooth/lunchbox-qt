...
| MED-004 | Per-game images and image-type selection | DV images page/image downloader; PA `ImageTypes` catalogue; the port's read-only index retains every configured image family using persisted front-image/region priorities and renders native file URLs in both shells plus selectable multi-image details galleries. Both frontends filter that index into a dedicated image-only viewer with direct/details entry, previous/next image-type switching, zoom/fit/pan, and focus-preserving Back behavior; LaunchBox keeps the modal in the owning docked or native pop-out details window. Bounded/symlink-safe scanning and real offscreen SVG decode/render gates cover front, gameplay screenshot, and background fixtures without writes. Image editing, downloads, box flip, and media mutation stay open |
...
| DESK-004 | Image zoom, type switching, box flip, full-screen images/models | DM zoom/image/model/flip actions; the first port subset implements image-only full-screen views in LaunchBox and BigBox over native Qt file URLs, wraparound image-family switching, an explicit 100–400%/25% zoom policy, fit reset, bounded pan, wheel/drag/keyboard/button input, and focus-preserving return-to-details behavior. LaunchBox supports docked and native pop-out ownership plus direct `I` entry. Box flip, 3D models, native game-controller bindings, and native Windows/macOS interaction stay open |
...
| BB-004 | Game details, images, video, 3D model, related games, and additional documents/apps | BV details/media/popups/actions; the first two port subsets open a keyboard-first full-screen details layer and a separate image-only layer from the selected stable-ID game. Details presents title/platform/status/ratings/statistics/metadata/notes beside actual image/video thumbnails, wraparound previous/next, autoplay, play/pause, mute, Enter-to-play, and Back over the shared bounded media index. The image layer adds direct and nested entry, video filtering, image-type switching, bounded zoom/fit/pan, wheel/drag/directional/page/Enter input, and context-preserving Back. Compiled 1280x720 scenarios drive the real controls through SVG and H.264 states, visually check both renders, validate native regular files in Rust, and hash the complete media tree. Native game-controller binding, box flip, 3D models, related games, documents, additional-app actions, custom theme layouts/transitions, and native Windows/macOS interaction remain open |
