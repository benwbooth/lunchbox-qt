...
| DESK-004 | Image zoom, type switching, box flip, full-screen images/models | DM zoom/image/model/flip actions; the first port subset implements BigBox's image-only full-screen view over native Qt file URLs, wraparound image-family switching, an explicit 100–400%/25% zoom policy, fit reset, bounded pan, wheel/drag/keyboard/button input, and return-to-details behavior. Box flip, 3D models, LaunchBox desktop parity, native game-controller bindings, and native Windows/macOS interaction stay open |
