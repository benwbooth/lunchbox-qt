...
| BB-004 | Game details, images, video, 3D model, related games, and additional documents/apps | BV details/media/popups/actions; the first four port subsets open a keyboard-first full-screen details layer and a separate image-only layer from the selected stable-ID game, add front/back box flipping, and add the shared interactive typed model viewer. Details presents title/platform/status/ratings/statistics/metadata/notes beside actual image/video thumbnails, wraparound previous/next, autoplay, play/pause, mute, Enter-to-play, and Back over the shared bounded media index. The image layer adds direct and nested entry, video filtering, image-type switching, bounded zoom/fit/pan, wheel/drag/directional/page/Enter input, and context-preserving Back. The wheel honors `ShowGameMenuFlipBox`; the game menu honors `ShowGameMenuViewModelFullscreen` and opens a six-face Qt Quick 3D model with `M`, visible controls, mouse/wheel/keyboard input, and strict persisted rotation lock shared with LaunchBox. Resolved 13.27 model settings select functional box, DVD, jewel, or long-jewel proportions and colors and choose separate face media or ratio-based `Box - Full` construction. The shared desktop platform editor atomically updates the platform override seen by BigBox. Compiled 1280x720 scenarios drive the real controls through SVG, H.264, and Qt Quick 3D states; require game-over-platform model precedence, the expected jewel dimensions, and the full-scan ratio; visually check details, image viewer, back cover, and model renders; validate native regular files in Rust; and hash the complete media tree. Native game-controller binding, logo/front-spine/rotation material rendering, exact protected mesh/material/transition/camera behavior, related games, documents, additional-app actions, custom theme layouts/transitions, and native Windows/macOS interaction remain open |
