...
| BB-004 | Game details, images, video, 3D model, related games, and additional documents/apps | BV details/media/popups/actions; the first port subset opens a dedicated keyboard-first full-screen layer with controller-shaped actions from the selected stable-ID game, presents title/platform/status/ratings/statistics/metadata/notes beside a large image/video presenter, and exposes actual thumbnails, wraparound previous/next, autoplay, play/pause, mute, Enter-to-play, and back behavior over the shared bounded media index. A compiled 1280x720 scenario drives the real Details, Previous, play/pause, and thumbnail controls through image and H.264 video states while hashing the complete media tree. Native game-controller binding, 3D models, related games, documents, additional-app actions, custom theme layouts/transitions, and native Windows/macOS interaction remain open |
