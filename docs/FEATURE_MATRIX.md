...
| BB-017 | Favorites, star ratings, playlists, random/discovery, and related games | BV popups/menu actions/discovery model. The current native game-action subsets recover six observed favorite/rating defaults plus the omitted-but-getter-defaulted playlist-action setting, preserve independent integer/floating ratings, use explicit half-star and append-order clean-room policies, and stable-partition favorites first. Wheel/details/menu Favorite and Rate actions honor their settings and recovered permissions. Add/Remove Playlist exposes only real membership changes on manual, non-generated playlists; the recovered dark list popup and five-star popup accept keyboard, mapped-controller, and pointer input. Favorite/rating uses one lossless platform transaction; membership uses a typed `PlaylistGame` and one lossless auxiliary-document transaction. Both retain exact backups and unknown XML, reread committed state, and publish live revisions. Membership fails closed while locked because no matching recovered permission exists. Pure policy/storage/controller/security tests and two compiled locked-to-unlocked interactions validate denial, rendered popups, rating/favorite changes, manual add/navigation/remove, `ManualOrder=-1`, exact transaction states, derived-playlist immutability, and no recovery manifests. The shared Rust/CXX-Qt/QML path contains no runtime shell, Windows path logic, or OS-specific UI branch. Exact protected stepping/pointer rounding/append-order algorithms, custom-theme bindings, discovery behavior, related games, and native Windows/macOS Qt interaction remain open |
