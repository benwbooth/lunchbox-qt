...
| RUN-012 | Game save management, version history, backup, restore, and deletion | DV saves/history; desktop save commands; PA save contracts; port subset models every persisted 13.27 `GameSave` field, provides a lossless Qt group/version inventory with transactional rename/combine/split metadata operations, manually backs up one resolved regular active file, restores a compatible regular vault version only after committing/verifying the current active bytes, and transactionally deletes one resolved regular vault copy plus its history row with exact recovery copies; emulator scanning, directory/container/companion backup, emulator-specific restore, active/container/companion-set deletion, automatic policy, repair, and adapters remain open |
