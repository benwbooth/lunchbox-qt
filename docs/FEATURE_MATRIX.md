...
The first `RUN-009` subset resolves the effective startup-screen enable flag
and millisecond delay from the actual primary target. A game override wins over
the effective emulator default; non-emulator targets use game settings. The
validated sequence carries that immutable policy into a shared LaunchBox and
BigBox Qt Quick overlay, and both frontends prove pre-start, primary-start,
timer-dismissal, child supervision, exact arguments, and statistics writes
with a real process. Theme/media selection, global minimum duration,
window/focus behavior, detached descendants, shutdown, and pause remain open.
...
| RUN-009 | Startup screens and per-emulator/game overrides | DV startup settings/edit pages; shared startup views; the first port subset resolves an immutable effective startup policy with recovered game-override/emulator-default precedence, carries it on the validated launch sequence, and drives one shared Qt Quick overlay in LaunchBox and BigBox from a pre-start notification through primary-process start and the exact stored millisecond delay. Direct, DOSBox, and ScummVM launches use the owning game's settings. Both-shell offscreen process smokes prove both visible phases, capture valid rendered PNGs, enforce timer dismissal before child exit, compare exact argv, and verify normal session-stat persistence. Theme/media selection, minimum-display policy, window/focus hiding, detached descendants, shutdown, and pause remain open |
