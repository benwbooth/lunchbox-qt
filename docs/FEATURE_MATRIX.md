...
| BB-007 | Startup video and startup/shutdown screens | BV application startup video plus the shared per-game startup view. The first application-startup subset indexes the recovered randomized direct-file `Videos/Startup` collection with exact legacy `Videos/Startup.mp4` fallback through the bounded cross-platform media boundary. BigBox keeps the main screen covered while loading, selects a native local URL, applies typed `VolumeVideo`, blocks background/game music, plays through Qt Multimedia, accepts one shared key/tap skip action, completes on natural end, and fails open on decode errors. Pure tests cover precedence, stable ordering, extension/size bounds, malformed volume, and symlink refusal. Three compiled runs use genuine H.264 to prove random-folder skip and natural completion plus legacy natural completion, render and color-check the video, validate exact safe files in Rust, and hash settings/media. Original splash image and sound packs, controller bindings, startup theme parity, primary-monitor routing, VLC/WMP backend parity, and native Windows/macOS multimedia remain open. Per-game startup/shutdown presentation is separately implemented under `RUN-009`/`RUN-011` |
