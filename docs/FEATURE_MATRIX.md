...
| MED-009 | 3D boxes, DVD/jewel cases, and full-screen model previews | DV model settings/previews; shared cover-flow model controls. The first port subset is a shared Qt Quick 3D full-screen generic box preview in LaunchBox and BigBox. It maps distinct selected front, back, and spine images onto six real faces; provides mouse, wheel, keyboard, and visible-button rotation/translation/zoom/reset; and atomically persists strict free/horizontal/vertical rotation-lock state in the native per-user configuration location. LaunchBox honors `ShowDetails3dModel` and opens from Details or `M`; BigBox honors `ShowGameMenuViewModelFullscreen` and opens from its game menu or `M`. Sequential compiled interactions prove cross-frontend state restoration/replacement, actual control wiring, decoded native regular textures, focus return, rendered PNGs, and byte-identical media/XML. DVD, jewel, and long-jewel geometry; full-scan image construction; model color/size/rotation and per-platform/per-game overrides; `Use3dModelImageView`/CoverFlow integration; exact original camera/timing/default bindings; native game controllers; and native Windows/macOS interaction remain open |
...
| DESK-004 | Image zoom, type switching, box flip, full-screen images/models | DM zoom/image/model/flip actions; the first three port subsets implement image-only full-screen views, animated front/back flipping, and a shared interactive generic six-face box model. Image views use native Qt file URLs, wraparound image-family switching, an explicit 100–400%/25% zoom policy, fit reset, bounded pan, wheel/drag/keyboard/button input, and focus-preserving return-to-details behavior. LaunchBox supports docked and native pop-out ownership plus direct `I` entry. Both shells select the box back through recovered settings priority and expose real front/back controls, `F`, and a shared 220 ms Qt Y-axis transition. LaunchBox now honors `ShowDetails3dModel`; its Details button or `M` opens the front/back/spine-textured Qt Quick 3D model with rotate/translate/zoom/reset controls and persistent strict rotation lock. Compiled interactions activate and return through the real controls, render back art and model PNGs, validate distinct native regular files in Rust, and prove whole-tree immutability. Exact original transition/camera behavior, specialized model types and overrides, native game-controller bindings, and native Windows/macOS interaction stay open |
...
| BB-004 | Game details, images, video, 3D model, related games, and additional documents/apps | BV details/media/popups/actions; the first four port subsets open a keyboard-first full-screen details layer and a separate image-only layer from the selected stable-ID game, add front/back box flipping, and add the shared interactive generic box model. Details presents title/platform/status/ratings/statistics/metadata/notes beside actual image/video thumbnails, wraparound previous/next, autoplay, play/pause, mute, Enter-to-play, and Back over the shared bounded media index. The image layer adds direct and nested entry, video filtering, image-type switching, bounded zoom/fit/pan, wheel/drag/directional/page/Enter input, and context-preserving Back. The wheel honors `ShowGameMenuFlipBox`; the game menu honors `ShowGameMenuViewModelFullscreen` and opens a front/back/spine-textured six-face Qt Quick 3D model with `M`, visible controls, mouse/wheel/keyboard input, and strict persisted rotation lock shared with LaunchBox. Compiled 1280x720 scenarios drive the real controls through SVG, H.264, and Qt Quick 3D states; visually check details, image viewer, back cover, and model renders; validate native regular files in Rust; and hash the complete media tree. Native game-controller binding, specialized model geometry/settings, exact protected transition/camera behavior, related games, documents, additional-app actions, custom theme layouts/transitions, and native Windows/macOS interaction remain open |
