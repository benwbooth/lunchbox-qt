...
complete app plus six exact macOS framework links. Core-selection policy and
netplay remain open under `RUN-016`. The first
...
| RUN-016 | RetroArch core selection, install, and netplay | RetroArch adapter; DV/BV netplay; install RetroArch view. The `RUN-003` lifecycle now installs the exact matching stable cores archive with Windows/Linux RetroArch and records it separately; macOS has no corresponding stable cores archive and deliberately does not invent one. Per-platform core choice, individual core download/update/removal, online updater policy, and netplay remain open |
