...
| RUN-012 | Game save management, version history, backup, restore, and deletion | DV saves/history; desktop save commands; PA save contracts; port subset models every persisted 13.27 `GameSave` field, provides a lossless Qt group/version inventory with transactional rename/combine/split metadata operations, uses a native RetroArch adapter to discover and append regular saves, numbered/auto states, and grouped Saturn companion sets, backs up regular files or complete Saturn sets, restores a compatible regular file or complete Saturn set only after committing and verifying the current active version, and transactionally deletes one resolved regular vault copy or complete Saturn set plus its history row with exact recovery copies; Saturn restore revision-checks and atomically replaces/creates selected companions while retaining active members absent from the selected version like 13.27; evidence-backed Dolphin state/GameCube regular files share the backup-first restore and active-delete path while `dolphin:wii:` directory saves remain gated; active deletion first atomically replaces the active history row with a verified portable vault copy, then revision-checks and deletes the regular file or complete Saturn set with all-or-rollback sibling recovery copies, including explicitly mapped external paths; non-RetroArch scanning, directory/container backup, Dolphin Wii/PCSX2 container restore/deletion, stale-row reconciliation, automatic policy, repair, and remaining adapters remain open |
...
| RUN-017 | Emulator-specific adapters | separate BigPEmu, Dolphin, MAME, PCSX2, RetroArch, ScummVM, and Xemu assemblies; legacy built-in ScummVM host-path/argument adapter implemented with a native Linux runtime fixture; first platform-neutral `lb-integrations` adapter implements RetroArch config-driven save/state discovery, Windows/Linux/macOS core parsing, mapped paths, and Saturn companion grouping/signatures/backup/restore/active and vault deletion; recovered Dolphin ordinary-file copy/delete semantics now enable GameCube files and state rows while Wii directories stay explicitly gated |
