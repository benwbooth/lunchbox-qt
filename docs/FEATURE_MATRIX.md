...
| MED-009 | 3D boxes, DVD/jewel cases, and full-screen model previews | DV model settings/previews; shared cover-flow model controls. The port has one shared Qt Quick 3D full-screen viewer in LaunchBox and BigBox. It maps distinct selected front, back, and spine images onto six real faces; provides mouse, wheel, keyboard, and visible-button rotation/translation/zoom/reset; and atomically persists strict free/horizontal/vertical rotation-lock state in the native per-user configuration location. LaunchBox honors `ShowDetails3dModel` and opens from Details or `M`; BigBox honors `ShowGameMenuViewModelFullscreen` and opens from its game menu or `M`. A limited protected-runtime Wine probe recovered the exact four model keys, new/editor defaults, root XML schema, signed ARGB and semicolon-size encodings, whole-record game/platform/built-in precedence, and all 41 built-in platform mappings. Both platform readers and the catalog reader now retain typed settings and unknown future data; a platform-neutral Rust resolver gives CXX-Qt a versioned presentation without interpreting opaque LaunchBox resource strings as paths. QML applies colors and forced proportions and presents distinct functional box, DVD, jewel, and long-jewel shapes. Sequential compiled interactions require the fixture's jewel game override to beat its DVD platform override, require exact port geometry, prove cross-frontend state restoration/replacement and actual control wiring, decode native regular textures, return focus, render PNGs, and preserve media/XML. Full-scan construction, logo/front-spine/rotation rendering, settings editing, `Use3dModelImageView`/CoverFlow integration, exact original meshes/materials/camera/timing/default bindings, native game controllers, and native Windows/macOS interaction remain open |
...
| BB-004 | Game details, images, video, 3D model, related games, and additional documents/apps | BV details/media/popups/actions; the first four port subsets open a keyboard-first full-screen details layer and a separate image-only layer from the selected stable-ID game, add front/back box flipping, and add the shared interactive typed model viewer. Details presents title/platform/status/ratings/statistics/metadata/notes beside actual image/video thumbnails, wraparound previous/next, autoplay, play/pause, mute, Enter-to-play, and Back over the shared bounded media index. The image layer adds direct and nested entry, video filtering, image-type switching, bounded zoom/fit/pan, wheel/drag/directional/page/Enter input, and context-preserving Back. The wheel honors `ShowGameMenuFlipBox`; the game menu honors `ShowGameMenuViewModelFullscreen` and opens a front/back/spine-textured six-face Qt Quick 3D model with `M`, visible controls, mouse/wheel/keyboard input, and strict persisted rotation lock shared with LaunchBox. Resolved 13.27 model settings select functional box, DVD, jewel, or long-jewel proportions and colors. Compiled 1280x720 scenarios drive the real controls through SVG, H.264, and Qt Quick 3D states; require game-over-platform model precedence and the expected jewel dimensions; visually check details, image viewer, back cover, and model renders; validate native regular files in Rust; and hash the complete media tree. Native game-controller binding, full-scan/model-detail rendering, editable model settings, exact protected mesh/material/transition/camera behavior, related games, documents, additional-app actions, custom theme layouts/transitions, and native Windows/macOS interaction remain open |
