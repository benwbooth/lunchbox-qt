...
| Startup lifecycle presentation | First `RUN-009` runtime subset. The platform planner resolves startup settings once from the actual primary launch plan: an explicit game override wins, otherwise an effective emulator supplies its default, while direct/DOSBox/ScummVM targets use the game's settings. The immutable sequence carries enabled state, exact millisecond delay, and provenance across the worker/process boundary. The controller publishes distinct pre-start and primary-start phases, and one compiled Qt Quick overlay shared by LaunchBox and BigBox blocks frontend interaction until its timer expires or the child exits. A failed launch or library replacement clears presentation state without a detached sleeper or shell | Platform tests cover emulator-default, game-override, direct-game, and selected-additional-application inheritance. The real-process offscreen smoke runs both frontends against a 1.05-second emulator fixture with a 250 ms inherited delay, observes the overlay before and after primary start, captures and validates the rendered PNG from each frontend, proves timer dismissal while the child remains supervised, compares exact argv, and checks the resulting play count/time/last-played records. Startup theme/media selection, global minimum duration, window/focus hiding, delegated descendants, shutdown screens, and pause screens remain open |
...
The workspace currently has 256 passing Rust tests. Both QML frontends and the
shared startup overlay are compiled into the native binaries. Their
...
- The process-launch vertical supports direct executables and one
  ...
  full process-tree lifecycle, focus, remaining startup theme/media behavior,
  pause/shutdown screens, remaining save workflows, controller
