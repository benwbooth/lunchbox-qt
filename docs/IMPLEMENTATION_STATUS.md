...
manager. Its real-dialog scenario verifies generated identity, all recovered
typed fields, atomic default handoff, referenced-delete refusal, lexical paths,
exact backups, unknown XML, target isolation, and transaction cleanup. Emulator
binary discovery, installation, updating, and deletion remain open and are not
implied by configuration CRUD.

The workspace currently has 233 passing Rust tests. Both QML resources are
