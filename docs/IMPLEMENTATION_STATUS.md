...
| Emulator save adapter | New platform-neutral `lb-integrations` boundary plus a first explicit RetroArch adapter: shared launch-emulator selection and host-path resolution, native `retroarch.cfg` parsing, content/core-sorted save and state discovery, cross-platform core-library names, main/additional-application ownership, Saturn companion grouping, and 13.27-compatible composite signatures | Five adapter tests cover regular saves, numbered/auto states, content/core sorting, Windows drive mappings, Windows/Linux/macOS core syntax, Saturn primary/companion ordering, exact signature, case-insensitive members, and symlink refusal; controller tests prove append-only/idempotent XML persistence plus transactional three-member Saturn backup and vault deletion; a real offscreen save-manager button scan proves portable paths, owner/core/slot/full metadata, targeted revision notification, exact XML backup, unknown-element retention, and manifest cleanup |
...
The workspace currently has 198 passing Rust tests. Both QML resources are
