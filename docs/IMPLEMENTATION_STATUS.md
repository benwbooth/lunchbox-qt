...
| Lossless editing | DOM-backed platform and auxiliary documents, transactional typed game/platform/category/playlist metadata and hierarchy edits, source-indexed alternate-name/custom-field/platform-folder/parent/playlist-filter/playlist-game replacement, platform/category/playlist/game/auxiliary-record add/remove, unknown-element retention, serialization, and no-overwrite save; persisted paths remain lexical strings until the platform launch boundary | All ten auxiliary families structurally round-trip; retained repeated-metadata, platform-folder, parent, playlist-filter, and playlist-game rows keep their exact unknown children while additions/removals remain deterministic; category lifecycle atomically updates `Platforms.xml` and `Parents.xml`; playlist lifecycle atomically creates/edits/deletes its document with `Parents.xml`, optionally clears owned `ListCache.xml` rows, detaches direct children to root, and never edits games or media; launch and platform-folder edits preserve Windows separators and unknown XML; platform lifecycle owns exactly 51 default folder rows and never creates media directories; fast/lossless platform readers produce identical modeled fixture data |
...
| Repository transactions | Root-scoped lock, multi-document replace/create/delete staging, streamed creation from external files, pre/post-stage revision checks, durable version-2 manifest, exact per-file/deletion backups, automatic rollback, conservative crash recovery, and refusal to write past a pending manifest | Two-document replace/create/delete and streamed-ROM-plus-XML commits, exact golden semantic diff, stale source, pending-manifest refusal, injected partial mutation, simulated process death, recovery, external divergence, unversioned/out-of-root targets, filename collision, non-empty deletion, and manifest path escape are tested |
| Manual ROM import | Reusable background planner and three-page Qt workflow for multiple native files/folders, recursive discovery, extension filtering, file- or folder-derived editable titles, resolved-path duplicate detection across games and additional applications, portable destination collision checks, leave/copy/move policies, same-stem/different-extension companion copying, non-recursive PDF-manual discovery, portable title/year subfolders, platform-default/direct/explicit emulator selection, conservative complete-disc-set grouping, and read-only local LaunchBox metadata lookup; PDF matching prefers a case-insensitive ROM stem, accepts a sole folder candidate, and exposes ambiguity without guessing; the SQLite adapter canonicalizes platform aliases, uses the recovered comparison and qualifier preference, auto-applies one exact match, and exposes stable database-ID choices for ambiguous exact matches; preview and execution revalidate configured and metadata IDs, execution re-plans the exact selection and edited title before a batch write, streams all copied files and platform XML through one recovery manifest, never overwrites a destination, and removes move sources only after byte verification | Unit tests cover recursive/sorted discovery, filters, duplicate defaults, stale-preview refusal, canonical configured-emulator lookup, unknown/empty-emulator refusal, direct-launch sentinel handling, exact/sole/ambiguous PDF outcomes, portable and reverse-mapped `ManualPath`, exact/ambiguous/missing local-metadata outcomes, explicit candidate selection, removed-candidate refusal, typed 14-field metadata/manual persistence, complete versus incomplete/ambiguous disc sets, portable edited-title subfolders and existing case-variant directory refusal, same-stem discovery and whole-game collision refusal, per-disc companions, typed additional-application insertion, atomic multi-file copy, portable stored paths, and verified multi-file move cleanup; the offscreen real-dialog scenario uses an actual ambiguous SQLite fixture and PDF, selects database ID 4242 through the Qt review payload, collapses two disc files to one game, persists the selected match and manual, copies both same-stem companions plus the PDF into the metadata-derived title/year folder, pins the configured emulator on the game and both ordered disc records, refreshes the game/additional-app models, checks exact copied bytes and relative LaunchBox paths, retains an exact XML backup and unknown XML, preserves copy sources, and leaves no recovery manifest |
...
| Qt data boundary | Worker-thread indexing/writing/import-preview/import-execution/launching, catalog-backed zero-game platform indexing, flattened category/platform/playlist hierarchy with recursive counts and filters, per-game priority-sorted additional-app indexing, ordered alternate-name/custom-field indexing and guarded row accessors, versioned typed game/platform/category/playlist/import payloads, persisted host-mapping CRUD, stale-request generations, queued CXX-Qt result delivery, and a virtualized `QAbstractListModel` with 36 named identity/state/metadata/launch roles, reset notifications, targeted state/row notifications, a platform/navigation revision signal, and metadata-driven query recomputation | The current packaged release load smoke delivered 35,869 games, 16,752 additional applications, 54 playlists, 11 emulators, and 37 platforms in 2.227 seconds; generated QML metadata plus 36-role/filter, typed-edit/import payload, repeated-row, game/platform/category/playlist CRUD, import, path-mapping, and process-launch runtime smokes prevent silent binding drift |
| LaunchBox shell | Background library loading, nested category/platform/playlist sidebar and add/edit/delete dialogs, a three-page ROM location/options/editable-preview importer with emulator, local-metadata, PDF-manual, title/year-subfolder, same-name companion, and complete-disc-set selection, descriptive-metadata search, counts, game grid with Play and Launch With controls, scrollable game, platform, category, and playlist metadata/hierarchy/membership editors, alternate-name, custom-field, and launch-configuration editing, Host Paths mapping editor, launch/session status, pending-recovery/conflict/reference-blocker banners, transactional metadata, launch configuration, and play-statistics editing, and playlist/category/platform/game add/remove | Offscreen role smoke; temporary-library state/rich-metadata/repeated-metadata/launch, dialog-driven ROM import, game/platform/category/playlist CRUD, and mapping CRUD scenarios; the import dialog proves native file input, configured-emulator selection, ambiguous exact SQLite metadata review by stable database ID, PDF candidate evidence and portable `ManualPath`, typed imported metadata, complete two-disc grouping, metadata-derived title/year placement, same-stem per-disc companion/manual copying, planner-backed editable preview, portable copied paths, exact streamed bytes, source preservation, game/additional-app model refresh, exact backup, and completed-manifest cleanup; the playlist dialog proves stable ID and immutable unique name, mutable display/BigBox metadata, manual and automatic membership, recovered filter grouping, nested rendering, cycle rejection, exact backup chains, direct-child detachment, portable filenames, lexical paths, exact backups, and media isolation; category and platform dialogs retain their corresponding hierarchy, folder, reference, and media-isolation gates; editor submission runs through real dialogs and versioned, unknown-field-denying typed payloads; process smokes prove path mapping, archive/M3U preparation, DOSBox/ScummVM semantics, lifecycle order, child reaping, play-statistics updates, completed-manifest cleanup, and unknown XML preservation |
...
The workspace currently has 164 passing Rust tests. Both QML resources are
...
- The manual ROM importer covers the recovered location, platform, emulator,
  ...
  canonicalizes platform aliases, reproduces the recovered exact comparison
  and parenthetical qualifier preference, auto-applies a unique match, exposes
  compact stable-ID choices for multiple exact matches, revalidates the chosen
  ID at execution, and persists the selected match's typed
  text/player/date/URL/rating fields. The recovered PDF option scans the main
  game folder without
  recursing, prefers a case-insensitive imported-file stem, accepts a sole
  ...
  Fuzzy or partial matching, metadata/online-media acquisition, emulator
  installation/editing and BIOS/core
  handling, descriptor-content dependency copying, metadata/database-driven
  version combining, broader disc-name grammars, MAME/FBNeo branches,
  cancellation/partial retention, and exact 13.27 duplicate/error rules remain
  open.
