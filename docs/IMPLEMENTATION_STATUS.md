...
| Lossless editing | DOM-backed platform and auxiliary documents, transactional typed descriptive/state/launch-configuration edits, source-indexed alternate-name/custom-field replacement, platform/game/auxiliary-record add/remove, unknown-element retention, serialization, and no-overwrite save; persisted paths remain lexical strings until the platform launch boundary | All ten auxiliary families structurally round-trip; retained repeated-metadata rows keep their exact unknown children while additions/removals remain deterministic; launch edits preserve Windows separators and unknown XML; platform lifecycle preserves unknown catalog nodes, owns exactly 51 default folder rows, and never creates media directories; fast/lossless platform readers produce identical modeled fixture data |
...
| Qt data boundary | Worker-thread indexing/writing/launching, catalog-backed zero-game platform indexing, per-game priority-sorted additional-app indexing, ordered alternate-name/custom-field indexing and guarded row accessors, persisted host-mapping CRUD, stale-request generations, queued CXX-Qt result delivery, and a virtualized `QAbstractListModel` with 36 named identity/state/metadata/launch roles, reset notifications, targeted state/row notifications, a platform revision signal, and metadata-driven query recomputation | The current packaged release load smoke delivered 35,869 games, 16,752 additional applications, 54 playlists, 11 emulators, and 37 platforms in 2.227 seconds; generated QML metadata plus 36-role/filter, typed-edit-payload, repeated-row, game/platform CRUD, path-mapping, and process-launch runtime smokes prevent silent binding drift |
| LaunchBox shell | Background library loading, catalog-backed platform sidebar and add/delete dialogs, descriptive-metadata search, counts, game grid with Play and Launch With controls, scrollable rich-metadata, alternate-name, custom-field, and launch-configuration editor, Host Paths mapping editor, launch/session status, pending-recovery/conflict/reference-blocker banners, transactional metadata, launch configuration, and play-statistics editing, and platform/game add/remove | Offscreen role smoke; temporary-library state/rich-metadata/repeated-metadata/launch, game/platform CRUD, and mapping CRUD scenarios; the platform dialog creates a portable coerced filename with 51 lexical folder rows, accepts a first game while empty, blocks referenced deletion, retains exact catalog/platform backups, deletes no media, and removes cleanly; editor submission runs through the real dialog and a versioned, unknown-field-denying typed payload; retained alternate/custom rows preserve unknown XML and new rows survive a fresh-process reload; process smokes prove path mapping, archive/M3U preparation, DOSBox/ScummVM semantics, lifecycle order, child reaping, play-statistics updates, completed-manifest cleanup, and unknown XML preservation |
...
The workspace currently has 117 passing Rust tests. Both QML resources are
...
- Add Game targets any catalog-backed platform document, including a newly
  ...
  or creating those stored media paths. Game and platform deletion are
...
desktop rich metadata/state, reference-gated game/platform add/remove, and first native Linux
