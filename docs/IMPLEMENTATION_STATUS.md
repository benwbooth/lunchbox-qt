...
| Lossless editing | DOM-backed platform and auxiliary documents, transactional typed game/platform/category metadata and hierarchy edits, source-indexed alternate-name/custom-field/platform-folder/parent replacement, platform/category/game/auxiliary-record add/remove, unknown-element retention, serialization, and no-overwrite save; persisted paths remain lexical strings until the platform launch boundary | All ten auxiliary families structurally round-trip; retained repeated-metadata, platform-folder, and parent rows keep their exact unknown children while additions/removals remain deterministic; category lifecycle atomically updates `Platforms.xml` and `Parents.xml`, detaches direct children to root on delete, and never deletes platforms, playlists, games, or media; launch and platform-folder edits preserve Windows separators and unknown XML; platform lifecycle owns exactly 51 default folder rows and never creates media directories; fast/lossless platform readers produce identical modeled fixture data |
...
| Qt data boundary | Worker-thread indexing/writing/launching, catalog-backed zero-game platform indexing, flattened category/platform hierarchy with recursive counts and filters, per-game priority-sorted additional-app indexing, ordered alternate-name/custom-field indexing and guarded row accessors, versioned typed game/platform/category edit payloads, persisted host-mapping CRUD, stale-request generations, queued CXX-Qt result delivery, and a virtualized `QAbstractListModel` with 36 named identity/state/metadata/launch roles, reset notifications, targeted state/row notifications, a platform/navigation revision signal, and metadata-driven query recomputation | The current packaged release load smoke delivered 35,869 games, 16,752 additional applications, 54 playlists, 11 emulators, and 37 platforms in 2.227 seconds; generated QML metadata plus 36-role/filter, typed-edit-payload, repeated-row, game/platform/category CRUD, path-mapping, and process-launch runtime smokes prevent silent binding drift |
| LaunchBox shell | Background library loading, nested category/platform sidebar and add/edit/delete dialogs, descriptive-metadata search, counts, game grid with Play and Launch With controls, scrollable game, platform, and category metadata/hierarchy editors, alternate-name, custom-field, and launch-configuration editing, Host Paths mapping editor, launch/session status, pending-recovery/conflict/reference-blocker banners, transactional metadata, launch configuration, and play-statistics editing, and category/platform/game add/remove | Offscreen role smoke; temporary-library state/rich-metadata/repeated-metadata/launch, game/platform/category CRUD, and mapping CRUD scenarios; the category dialog proves nested rendering, recursive counts/filtering, immutable identity, cycle rejection, multiple placements, paired exact backup chains, direct-child detachment, unknown XML retention, and media isolation; the platform dialogs create a portable coerced filename with 51 lexical folder rows, transactionally edit and reload metadata plus ordered folders, accept a first game while empty, block referenced deletion, retain the exact create/edit/delete backup chain, delete no media, and remove cleanly; editor submission runs through real dialogs and versioned, unknown-field-denying typed payloads; process smokes prove path mapping, archive/M3U preparation, DOSBox/ScummVM semantics, lifecycle order, child reaping, play-statistics updates, completed-manifest cleanup, and unknown XML preservation |
...
The workspace currently has 128 passing Rust tests. Both QML resources are
...
desktop rich metadata/state, existing-platform metadata/folder editing,
...
playlist CRUD and reviewed dependency-remediation choices, the remaining launch lifecycle breadth is implemented,
