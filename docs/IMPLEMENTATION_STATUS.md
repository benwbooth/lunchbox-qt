...
| LaunchBox shell | Background library loading, nested category/platform/playlist sidebar and add/edit/delete dialogs, a three-page ROM location/options/editable-preview importer with emulator, local-metadata, PDF-manual, title/year-subfolder, same-name companion, complete-disc-set, and matching-title version selection, descriptive-metadata search, counts, game grid with Play, Launch With, per-game Apps, and per-game Saves controls, scrollable game, additional-application, save-group/version, platform, category, and playlist metadata/hierarchy/membership editors, alternate-name, custom-field, and launch-configuration editing, Host Paths mapping editor, launch/session status, pending-recovery/conflict/reference-blocker banners, transactional metadata, launch configuration, additional-application Make Default, manual regular-file save backup/restore, confirmed regular-file vault deletion, and play-statistics editing, and playlist/category/platform/game/additional-application add/remove | Offscreen role smoke; temporary-library state/rich-metadata/repeated-metadata/launch, dialog-driven ROM import, game/additional-application/save/platform/category/playlist CRUD, Make Default, and mapping CRUD scenarios; the save manager displays recovered group/version history, labels only resolved paths Active or Vault, leaves unmapped Windows paths Unresolved, and proves rename-version, rename-group, combine, and Make New Save through four real dialogs/transactions without touching files; a separate manager-driven smoke backs up one regular active file to `Saves/<Platform>/<ROM name>.<ext>`, retains exact active bytes, persists size/seven-digit UTC time/MD5 and an explicit legacy group, keeps one exact XML rollback backup, and leaves no manifest; a confirmation-dialog restore smoke requires one compatible active row, first commits/verifies its current bytes as a collision-free new vault version, then revision-checks and atomically replaces the active file from the selected vault copy while retaining an exact sibling recovery copy; another real confirmation-dialog smoke deletes one resolved regular vault file plus its row, retains exact file/XML recovery copies, preserves the active file and unknown XML, refreshes the targeted model, and leaves no manifest; directory/container and companion-file backup, scanning, emulator-specific restore, active/container/companion-set deletion, automatic backup settings, repair, and adapter execution remain open; the additional-application dialog proves the recovered editable field set, direct/emulator canonicalization, lexical Windows paths, exact edit/add/delete and edit/default backup chains, targeted model refresh, immutable identity/owner/provider/cloud state, retained app records, reference-gated deletion, unknown XML retention, and target/media isolation; the import dialog proves native file input, configured-emulator selection, exact-to-partial SQLite fallback and ambiguous partial review by stable database ID, PDF candidate evidence and portable `ManualPath`, typed imported metadata, complete two-disc grouping, exact database-driven USA/World version grouping, primary and secondary version chooser records, corrected metadata-title/year placement, same-stem per-disc companion/manual copying, planner-backed editable preview, portable copied paths, exact streamed bytes, source preservation, game/additional-app model refresh, exact backup, and completed-manifest cleanup; the playlist dialog proves stable ID and immutable unique name, mutable display/BigBox metadata, manual and automatic membership, recovered filter grouping, nested rendering, cycle rejection, exact backup chains, direct-child detachment, portable filenames, lexical paths, exact backups, and media isolation; category and platform dialogs retain their corresponding hierarchy, folder, reference, and media-isolation gates; editor submission runs through real dialogs and versioned, unknown-field-denying typed payloads; process smokes prove path mapping, archive/M3U preparation, DOSBox/ScummVM semantics, lifecycle order, child reaping, play-statistics updates, completed-manifest cleanup, and unknown XML preservation |
...
The workspace currently has 191 passing Rust tests. Both QML resources are
