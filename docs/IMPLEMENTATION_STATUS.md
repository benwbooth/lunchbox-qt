...
| Manual ROM import | Reusable background planner and three-page Qt workflow for multiple native files/folders, recursive discovery, extension filtering, file- or folder-derived editable titles, resolved-path duplicate detection, portable destination collision checks, leave/copy/move policies, and platform-default/direct/explicit emulator selection; preview and execution revalidate configured IDs against `Emulators.xml`, execution re-plans the exact selection before a batch write, streams copied files and platform XML through one recovery manifest, never overwrites a destination, and removes move sources only after byte verification | Unit tests cover recursive/sorted discovery, filters, duplicate defaults, stale-preview refusal, canonical configured-emulator lookup, unknown/empty-emulator refusal, direct-launch sentinel handling, atomic two-ROM copy, portable stored paths, and verified move cleanup; the offscreen real-dialog scenario previews two native files, pins the configured emulator on both imported games, writes five total model rows, checks exact copied bytes and relative LaunchBox paths, retains an exact XML backup and unknown XML, preserves copy sources, and leaves no recovery manifest |
...
The workspace currently has 146 passing Rust tests. Both QML resources are
...
- The manual ROM importer covers the recovered location, platform, emulator,
  ...
  destination overwrites. Copy/move
  ...
  handling, related-file copying, multi-disc combining, MAME/FBNeo branches,
