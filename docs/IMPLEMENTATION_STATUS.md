...
| Emulator save adapter | New platform-neutral `lb-integrations` boundary plus a first explicit RetroArch adapter: shared launch-emulator selection and host-path resolution, native `retroarch.cfg` parsing, content/core-sorted save and state discovery, cross-platform core-library names, main/additional-application ownership, Saturn companion grouping, and 13.27-compatible composite signatures and companion replacement rules | Five adapter tests cover regular saves, numbered/auto states, content/core sorting, Windows drive mappings, Windows/Linux/macOS core syntax, Saturn primary/companion ordering, exact signature, case-insensitive members, and symlink refusal; controller tests prove append-only/idempotent XML persistence plus transactional three-member Saturn backup, restore, and vault deletion; real offscreen manager actions prove discovery and dialog-confirmed full-set restore with portable paths, owner/core/slot/full metadata, mandatory verified backup, targeted revision notification, exact file/XML recovery copies, unknown-element retention, and manifest cleanup |
...
The workspace currently has 201 passing Rust tests. Both QML resources are
