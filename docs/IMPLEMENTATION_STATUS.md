...
| Lossless editing | DOM-backed platform and auxiliary documents, transactional typed game/additional-application/platform/category/playlist metadata and hierarchy edits, source-indexed alternate-name/custom-field/platform-folder/parent/playlist-filter/playlist-game replacement, platform/category/playlist/game/additional-application/auxiliary-record add/remove, unknown-element retention, serialization, and no-overwrite save; persisted paths remain lexical strings until the platform launch boundary | All ten auxiliary families structurally round-trip; retained repeated-metadata, additional-application, platform-folder, parent, playlist-filter, and playlist-game rows keep their exact unknown children while additions/removals remain deterministic; additional-application edits retain immutable identity/owner/storefront/cloud state and deletion refuses game-save references without deleting targets or media; category lifecycle atomically updates `Platforms.xml` and `Parents.xml`; playlist lifecycle atomically creates/edits/deletes its document with `Parents.xml`, optionally clears owned `ListCache.xml` rows, detaches direct children to root, and never edits games or media; launch and platform-folder edits preserve Windows separators and unknown XML; platform lifecycle owns exactly 51 default folder rows and never creates media directories; fast/lossless platform readers produce identical modeled fixture data |
...
| LaunchBox shell | Background library loading, nested category/platform/playlist sidebar and add/edit/delete dialogs, a three-page ROM location/options/editable-preview importer with emulator, local-metadata, PDF-manual, title/year-subfolder, same-name companion, complete-disc-set, and matching-title version selection, descriptive-metadata search, counts, game grid with Play, Launch With, and per-game Apps controls, scrollable game, additional-application, platform, category, and playlist metadata/hierarchy/membership editors, alternate-name, custom-field, and launch-configuration editing, Host Paths mapping editor, launch/session status, pending-recovery/conflict/reference-blocker banners, transactional metadata, launch configuration, and play-statistics editing, and playlist/category/platform/game/additional-application add/remove | Offscreen role smoke; temporary-library state/rich-metadata/repeated-metadata/launch, dialog-driven ROM import, game/additional-application/platform/category/playlist CRUD, and mapping CRUD scenarios; the additional-application dialog proves the recovered editable field set, direct/emulator canonicalization, lexical Windows paths, exact edit/add/delete backup chain, targeted model refresh, immutable identity/owner/provider/cloud state, reference-gated deletion, unknown XML retention, and target/media isolation; the import dialog proves native file input, configured-emulator selection, exact-to-partial SQLite fallback and ambiguous partial review by stable database ID, PDF candidate evidence and portable `ManualPath`, typed imported metadata, complete two-disc grouping, exact database-driven USA/World version grouping, primary and secondary version chooser records, corrected metadata-title/year placement, same-stem per-disc companion/manual copying, planner-backed editable preview, portable copied paths, exact streamed bytes, source preservation, game/additional-app model refresh, exact backup, and completed-manifest cleanup; the playlist dialog proves stable ID and immutable unique name, mutable display/BigBox metadata, manual and automatic membership, recovered filter grouping, nested rendering, cycle rejection, exact backup chains, direct-child detachment, portable filenames, lexical paths, exact backups, and media isolation; category and platform dialogs retain their corresponding hierarchy, folder, reference, and media-isolation gates; editor submission runs through real dialogs and versioned, unknown-field-denying typed payloads; process smokes prove path mapping, archive/M3U preparation, DOSBox/ScummVM semantics, lifecycle order, child reaping, play-statistics updates, completed-manifest cleanup, and unknown XML preservation |
...
The workspace currently has 173 passing Rust tests. Both QML resources are
