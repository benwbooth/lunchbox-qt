...
| Manual ROM import | Reusable background planner and three-page Qt workflow for multiple native files/folders, recursive discovery, extension filtering, file- or folder-derived editable titles, resolved-path duplicate detection across games and additional applications, portable destination collision checks, leave/copy/move policies, same-stem/different-extension companion copying, platform-default/direct/explicit emulator selection, and conservative complete-disc-set grouping; preview and execution revalidate configured IDs against `Emulators.xml`, execution re-plans the exact selection before a batch write, streams all copied files and platform XML through one recovery manifest, never overwrites a destination, and removes move sources only after byte verification | Unit tests cover recursive/sorted discovery, filters, duplicate defaults, stale-preview refusal, canonical configured-emulator lookup, unknown/empty-emulator refusal, direct-launch sentinel handling, complete versus incomplete/ambiguous disc sets, same-stem discovery and whole-game collision refusal, per-disc companions, typed additional-application insertion, atomic multi-file copy, portable stored paths, and verified multi-file move cleanup; the offscreen real-dialog scenario collapses two disc files to one game, copies both same-stem companions, pins the configured emulator on the game and both ordered disc records, refreshes the game/additional-app models, checks exact copied bytes and relative LaunchBox paths, retains an exact XML backup and unknown XML, preserves copy sources, and leaves no recovery manifest |
...
The workspace currently has 151 passing Rust tests. Both QML resources are
...
- The manual ROM importer covers the recovered location, platform, emulator,
  ...
  targets are portable `Games\<platform>` paths on every host; leave-in-place
  paths become library-relative where possible, reverse explicit Windows
  ...
  and the source is retained with a warning. LaunchBox database matching,
  metadata/media acquisition, emulator installation/editing and BIOS/core
